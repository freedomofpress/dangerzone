...
=== "Qubes OS"
    ...
    > ⚠️ Native Qubes support is in beta stage, so the instructions below
    ...
    2. Create an offline disposable template (app qube) called `dz-dvm`, based on the `fedora-43-dz` template. This will be the qube where the documents will be sanitized:
    ...
    3. Create an **app** qube (`dz`) that will be used for Dangerzone development and initiating the sanitization process:
        ...
        > 💡 Alternatively, you can use a different app qube for Dangerzone
    ...
    4. Add an RPC policy (`/etc/qubes/policy.d/50-dangerzone.policy`) that will allow launching a disposable qube (`dz-dvm`) when Dangerzone converts a document, with the following contents:
    ...
    1. Install the `.rpm` packages you just copied, in order to get the Dangerzone dependencies:
