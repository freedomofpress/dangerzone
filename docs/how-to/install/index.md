...
=== "macOS"
    ...
    release = f"https://github.com/freedomofpress/dangerzone/releases/download/v{version}"
    cog.outl("")  # the list must not touch the cog comment, or it will not render
    cog.outl(f"    - Download [Dangerzone {version} for Mac (Apple Silicon CPU)]({release}/Dangerzone-{version}-arm64.dmg)")
    cog.outl(f"    - Download [Dangerzone {version} for Mac (Intel CPU)]({release}/Dangerzone-{version}-i686.dmg)")
    ]]] -->
...
=== "Windows"
    ...
    <!-- [[[cog
    cog.outl("")  # version and release come from the cog block above
    cog.outl(f"    - Download [Dangerzone {version} for Windows]({release}/Dangerzone-{version}.msi)")
    ]]] -->
    ...
    ??? info "See which versions of Windows are supported"
        ...
        We generally support Windows releases that are within [Microsoft’s servicing timeline](https://support.microsoft.com/en-us/help/13853/windows-lifecycle-fact-sheet). That means that a recent release of Windows 10 or Windows 11 is required. In addition, you need to have hardware virtualization enabled (to be more specific, on x64, WSL requires build 18362 or later, and 19041 or later is required for arm64 systems). If you are running Windows in a virtual machine, the VM must support nested virtualization. These requirements come from [Podman](https://github.com/containers/podman/blob/v5.8.1/docs/tutorials/podman-for-windows.md#prerequisites), which Dangerzone uses under the hood.
...
*If you prefer to build Dangerzone from source, follow the [run Dangerzone from source tutorial](../../tutorials/run-dangerzone-from-source.md).*
