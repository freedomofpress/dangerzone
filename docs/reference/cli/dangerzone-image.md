...
<!-- [[[cog
import subprocess
help_text = subprocess.run(["dangerzone-image", "--help"], capture_output=True, text=True, check=True).stdout.rstrip()
cog.out(f"""```console
{help_text}
```""")
]]] -->
...
Commands:
  ...
  store-signatures  Retrieves and stores the signatures of the remote...
...
<!-- [[[cog
help_text = subprocess.run(["dangerzone-image", "upgrade", "--help"], capture_output=True, text=True, check=True).stdout.rstrip()
cog.out(f"""```console
{help_text}
```""")
]]] -->
...
Usage: dangerzone-image upgrade [OPTIONS]
  ...
  To upgrade to a custom sandbox image, use "prepare-archive" and "load-
  archive" instead.
...
<!-- [[[cog
help_text = subprocess.run(["dangerzone-image", "verify-local", "--help"], capture_output=True, text=True, check=True).stdout.rstrip()
cog.out(f"""```console
{help_text}
```""")
]]] -->
...
<!-- [[[cog
help_text = subprocess.run(["dangerzone-image", "prepare-archive", "--help"], capture_output=True, text=True, check=True).stdout.rstrip()
cog.out(f"""```console
{help_text}
```""")
]]] -->
...
Options:
  ...
  --output TEXT  The location of the generated archive. '{arch}' will be
                 replaced by the specified or detected architecture (see
                 --arch)  [default: dangerzone-{arch}.tar]
...
<!-- [[[cog
help_text = subprocess.run(["dangerzone-image", "load-archive", "--help"], capture_output=True, text=True, check=True).stdout.rstrip()
cog.out(f"""```console
{help_text}
```""")
]]] -->
...
<!-- [[[cog
help_text = subprocess.run(["dangerzone-image", "store-signatures", "--help"], capture_output=True, text=True, check=True).stdout.rstrip()
cog.out(f"""```console
{help_text}
```""")
]]] -->
