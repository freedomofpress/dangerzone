...
def test_update_checks(
    ...
    report = releases.check_for_updates(settings)
    assert_report_equal(report, ReleaseReport(version="99.9.9", changelog="changelog"))
...
def test_update_errors(
    ...
    # Test 6 - Check that a non-string changelog is reported as an error
    ...
    assert type(report) is ErrorReport
    assert report.error is not None
    assert "Invalid changelog format" in report.error
